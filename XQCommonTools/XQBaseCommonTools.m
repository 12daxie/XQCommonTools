...
@implementation XQCommonTools
///工具的单例 singleton
+ (XQCommonTools *)sharedXQCommonTools {
    static XQCommonTools *aCommonTools;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        aCommonTools = [[XQCommonTools alloc] init];
    });
    return aCommonTools;
}
 ...
 */
- (NSString *)setXQDebugLogToFile {
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentDirectory = [paths objectAtIndex:0];
    NSString *fileName = [NSString stringWithFormat:@"XQNSLog.txt"];// 注意不是NSData!
